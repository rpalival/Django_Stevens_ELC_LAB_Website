...
from studentapp.models import UserInfo, Attendance
...
def bookSlot(request):
    ...
    return render(request, "bookSlot.html", {"attend_obj": attend_obj, "form": form})
...
def cancel_slot(request):
    ...
    if request.method == "GET":
        ...
        return render(
            request, "cancelSlot.html", {"attend_obj": attend_obj, "cform": cform}
        )
...
def teacherAttendance(request):
    ...
    if request.method == "GET":
        ...
        return render(
            request, "teacherAttendance.html", {"obj": attend_obj, "form": form}
        )
    ...
    if form.is_valid():
        ...
        new_obj = Attendance.objects.filter(
            studentID=newID, date=newDate, starttime=newTime
        )
        # print(new_obj)
        if not new_obj:
